...
int main(int argc, char* argv[])
{
    ...
    if (argc != 2)
    {
        cout << "Invalid input. This program takes One arguments: preset model specification (ie. "
                "Sanden80). Recieved input: \n";
...
int testIsTankSizeFixed(HPWH::MODELS model)
{
    ...
    { // it better have changed
        ...
        {
            cout << "Error, the tank size hasn't changed to the new tank size when it should have. "
                    "New Size: "
                 << newTankSize << ". Returned Value: " << hpwh.getTankSize() << "\n";
            return 1;
        }
    }
    return 0;
